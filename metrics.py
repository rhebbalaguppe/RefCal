...
def auc_m(y_true, y_pred, label1=None, label2=None, freq = False):
    ...
    #print(y_true.shape)
    try:
        return roc_auc_score(y_true,torch.nn.functional.softmax(y_pred,dim=1),multi_class='ovo')
...
def get_all_metrics(name, output, target, opt,n_bins = 15, logits = False):
    ...
    try:
        ...
        metrics[name+'_ace'] = -1
    try:
        conf, acc = rp.multiclass_logits_to_confidences(output, target)
        metrics[name+'_smece'] = rp.smECE(f=conf,y=acc)
    except:
        metrics[name+'_smece'] = -1
    try:
    ...
    if opt.n_cls==2:
        ...
        output_prob = torch.nn.functional.softmax(output,dim=1)[:,1]
        metrics[name+'_top1'] = float(accuracy(output, target, topk=(1,))[0][0])
        metrics[name+'_auc'] = roc_auc_score(target, output_prob)
    else:
        # TODO: 
        metrics[name+'_top1'] = float(accuracy(output, target, topk=(1,))[0][0])
        metrics[name+'_auc'] = auc_m(target, output)
