...
def testData(net1, criterion, CUDA_DEVICE, testloader10, testloader, nnName, dataName, noiseMagnitude1, temper,classifier,opt):
...
########################################In-distribution###########################################
    for j, data in enumerate(testloader10):
        ...
        if classifier == None:
            ...
            outputs = classifier(net1.encoder(inputs))
        ...
        nnOutputs = sm(nnOutputs).numpy()[0]
        ...
        f1.append([temper, noiseMagnitude1, np.max(nnOutputs)])

        if np.argmax(nnOutputs) == label:
            accuracy_count+=1
        ...
        if classifier == None:
            ...
            outputs = classifier(net1.encoder(Variable(tempInputs)))
    ...
    print("The accuracy is ", accuracy_count/N)
...
###################################Out-of-Distributions#####################################
    for j, data in enumerate(testloader):
        ...
        nnOutputs = np.exp(nnOutputs)/np.sum(np.exp(nnOutputs))
        #f2.write("{}, {}, {}\n".format(temper, noiseMagnitude1, np.max(nnOutputs)))
        f2.append([temper, noiseMagnitude1, np.max(nnOutputs)])
