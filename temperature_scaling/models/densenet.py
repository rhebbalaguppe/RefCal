"""
Ref: https://github.com/weiaicunzai/pytorch-cifar100/blob/master/models/densenet.py
...
class Bottleneck(nn.Module):
    def __init__(self, in_channels, growth_rate):
        ...
        self.bottle_neck = nn.Sequential(
            ...
            nn.Dropout(p=0.2) # Add as per the original implementation !
        )
...
def densenet264(num_classes):
    return DenseNet(Bottleneck, [6,12,64,48], growth_rate=32, num_class=num_classes)



def _get_densenet(model, dataset):
    assert "densenet" in model

    model_name = model
    model_name = str.replace(model, "densenet", "")
