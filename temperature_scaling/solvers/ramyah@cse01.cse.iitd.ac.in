...
class ACLSLoss(nn.Module):
    ...
    def forward(self, logits, target, temp=1.0):
        ...
        with torch.no_grad():
            ...
            true_dist.scatter_(self.dim, target.unsqueeze(self.dim), (1 - self.lambda1 * F.relu(logits - min_z_k - self.M)))

            # Compute the ACLS loss for j ≠ y^
            true_dist_other = target_one_hot * self.lambda2 * F.relu(min_z_k - logits - self.M)
